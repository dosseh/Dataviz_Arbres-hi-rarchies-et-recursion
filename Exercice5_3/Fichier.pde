...
class Fichier extends SimpleMapItem {
  ...
  ArrayList <Integer> colorByLastModified(String path) {
    ArrayList<Integer> recuvaNumber = new ArrayList();
    File[] files = listFiles(path);
    long[] tabFiles = new long[files.length];
    recuvaNumber.clear();
    for (int i = 0; i < files.length; i++) {
      File f = files[i]; 
      tabFiles[i] = f.lastModified();
      String lastModified = new Date(f.lastModified()).toString();
      println("Last Modified: " + lastModified + " |> "+  f.getName());
      println("-----------------------");
    }
    java.util.Arrays.sort(tabFiles);
    int i = 0;
    for (long thisFile : tabFiles) {
      println(thisFile + " Number "+ i++);
      recuvaNumber.add(i++);
    }
    return recuvaNumber;
  }
